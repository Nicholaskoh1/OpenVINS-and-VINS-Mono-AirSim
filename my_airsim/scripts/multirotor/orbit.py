...
class OrbitNavigator:
    def __init__(self, radius = 10, altitude = 1, speed = 0.1, iterations = 1, center = [1,0], snapshots = None):
        self.radius = radius
    ...
    def write_to_csv(self, gps_file):
        with open(gps_file, 'w', newline = '') as csvfile:
            ...
            for pos in self.positions: # assuming self.positions is a list of Position objects
                writer.writerow({'X': pos.x, 'Y': pos.y, 'Z': pos.z})
    ...
    def start(self):
        ...
        while count < self.iterations:
            ...
            if time.time() - new_start_time >= 0.1 and pos.z_val < -5:
                new_start_time = time.time()
                pos = self.client.getMultirotorState().kinematics_estimated.position
        ...
        if True:            
            ...
            print("descending")
            self.client.moveToPositionAsync(start.x_val, start.y_val, -1, 1).join()
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--radius", type=float, help="radius of the orbit", default= 50)
